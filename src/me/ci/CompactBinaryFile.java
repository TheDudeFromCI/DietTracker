...
public class CompactBinaryFile extends File{
	private static boolean bitAt(byte b, int pos){
		if(pos==0){
			return (b&128)==128;
		}
		if(pos==1){
	...
	public long getNumber(int bits){
		...
		if(bits<1){
			throw new IllegalArgumentException("Cannot return a number with less then 1 bit!");
		}
		if(bits>64){
			throw new IllegalArgumentException("Cannot return a number with more then 64 bits!");
		}
		long n = 0;
	...
	public void resetIterator(){
		if(!reading&&!writing){
			throw new IllegalStateException("Not editing file!");
		}
		pos = subPos = 0;
	}
	...
	public void addNumber(long number, int bits){
		...
		if(bits<1){
			throw new IllegalArgumentException("Cannot write a number with less then 1 bit!");
		}
		if(bits>64){
	...
	public CompactBinaryFile(File parent, String child){
		super(parent, child);
	}
	public CompactBinaryFile(String parent, String child){
		super(parent, child);
	}
	public CompactBinaryFile(URI uri){
		super(uri);
	}
	public CompactBinaryFile(String file){
		super(file);
	}
